...
function download_compose_file() {
    ...
    wget -q https://github.com/Dockerku/tools/raw/main/docker-compose.yml -O docker-compose.yml
}
...
function set_image_version() {
    if [ -n "$version" ]; then
        export version
        echo -e "${green}Will use version: puntung/rokok:$version ${plain}"
        sed -i "s/image:.*/image: puntung/rokok:$version/g" docker-compose.yml
    else
        echo -e "${green}Will use defalut version: puntung/rokok:latest ${plain}"
    fi
}
...
function set_proxy() {
    ...
    if [ ! -f ./proxychains4.conf ]; then
        ...
        wget -q https://github.com/Dockerku/tools/raw/main/proxychains4.conf -O proxychains4.conf
    else
